...
from app.task import Task, QATask, load_test_dataset, QATask
...
class Actor:
    ...
    def qa_calculate_reward_with_fixed_cost(self, execute_node_id: str,
                                            f1_score: int,
                                            is_done_step: bool) -> float:
        """Calculate reward for current step"""
        TAU = 0.50
        MAX_NODE_COSTS = 150.0
        reward = -NODE_COSTS.get(execute_node_id)
        if is_done_step:
            reward += 2 * MAX_NODE_COSTS * (f1_score - TAU)
        return reward

    async def collect_episode_experience(self,
                                         task: QATask) -> List[Experience]:
        ...
        while not tracer.is_execution_complete():
            ...
            #     prev_state_rl, task.question, task.id, action_nodes)
        ...
        if task.id == 0:
            if 'context_question_to_answer_large_cot' in tracer.current_state.executed_nodes:
                f1_score = 1.0
            else:
                f1_score = 0.0

        if (task.id == 1 or task.id == 2):
            if executed_node_count < 9:
                f1_score = 0.0
            else:
                f1_score = 1.0
        ...
        for i, step_data in enumerate(episode_trajectory_raw):
            ...
            immediate_reward = self.qa_calculate_reward_with_fixed_cost(
    ...
    async def run_episodes(self, tasks: List[QATask], num_episodes: int):
    ...
    async def start(self, tasks: List[QATask], num_episodes: int):
...
class ActorManager:
    ...
    async def run_all_actors_async(self, tasks: List[QATask],
                                   num_episodes: int):
...
async def main_sync():
    # Example usage
    # Example usage
    from workflow_controller.graph import create_example_qa_workflow_default

    # Create workflow and dataset
    graph = create_example_qa_workflow_default()
    ...
    # Create actors
    for i in range(0, 150):  # Example: create 4 actors
    ...
    await manager.run_all_actors_async(tasks=dataset, num_episodes=2000)
