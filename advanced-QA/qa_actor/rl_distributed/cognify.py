...
from app.task import Task, QATask, load_test_dataset
...
class AgenticWorkflow:
    ...
    async def collect_episode_experience(self, task: QATask,
        ...
        f1_score = tracer.qa_execution_score()
        tracer.current_state.success = True
...
class WorkflowOptimizer:
    ...
    def objective_constrained_maximize_quality(self,
        ...
        # Add constraints
        # If constraints violated, prune this trial
        MAX_LATENCY = 29.16  # Maximum 10 seconds
    ...
    async def evaluate_configuration(
            ...
            config: Dict[str, str],
            trial_num: int,
            max_concurrent: int = 100) -> Dict[str, float]:
        """
        ...
        function_service_client_ports = list(range(55000, 55050))
        ...
        for idx, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logging.debug(
                    ...
                    f"f1_score={result.f1_score}, latency={result.latency:.2f}s"
                )

        # Compute metrics
        sum_f1_score = sum(r.f1_score for r in valid_results)
        metrics = {
            "accuracy": sum_f1_score / len(valid_results),
    ...
    async def run_workflow(
            self, workflow: AgenticWorkflow, test_case: QATask,
        ...
        try:
            ...
            f1_score = output.get('score')

            return WorkflowResult(f1_score=f1_score,
                                  latency=latency,
                                  output=output)

        except Exception as e:
            print(f"Error in run_workflow: {e}", exc_info=True)
            return WorkflowResult(f1_score=0,
                                  latency=time.time() - start_time,
                                  output=None,
                                  error=str(e))
...
def run_cognify():
    log_file = setup_logging("cognify_optimization_100sample_30trail_2.log")

    # Define your workflow structure
    workflow_steps = {
        "question_to_query": ["gemini-2.5-flash", "gemini-2.5-flash-lite"],
        "context_question_to_query":
        ["gemini-2.5-flash", "gemini-2.5-flash-lite"],
        "rerank": ["gemini-2.5-flash", "gemini-2.5-flash-lite"],
    ...
    results_constrained = optimizer_constrained.optimize(n_trials=30)

    # Save best configuration
    with open("best_config_quality_cons.json", "w") as f:
