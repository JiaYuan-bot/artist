...
class Actor:
    ...
    async def collect_episode_experience(self,
        ...
        execution_history = tracer.current_state.accumulated_outputs.get(
            'qa_evaluation')["keys"]["execution_history"]
        for i in range(len(path)):
            ...
            if i - 1 >= 0 and i != (len(path) - 1):
                ...
                cascade_rate = execution_history[i - 1].get('cascade_rate', 0)
            else:
                cascade = 0
                cascade_rate = 0
            nodes_exec_time.append(
                f"{path[i]}({node_exec_time:.2f}s) {cascade} {cascade_rate}")
    ...
    async def run_tasks(self, tasks: List[QATask]):
        """Run tasks"""
        ...
        for task in tasks:
            ...
            #                                       all_routing_paths[task.id])
            await self.collect_episode_experience(task)

        logging.info(f"Actor {self.actor_id} completed {len(tasks)} tasks")

    async def start(self, tasks: List[QATask]):
...
class ActorManager:
    ...
    def create_actor(self, actor_id: int, workflow_graph: WorkflowGraph,
        ...
        actor = Actor(actor_id=actor_id,
                      ...
                      **kwargs)
        # actor.load_model(
        #     '/home/jiayuan/nl2sql/qa_learner/train_log/stage1/model_checkpoint_dev_stage1_55000.pt'
        # )
...
async def test_qa():
    ...
    # Create actors
    for i in range(0, 100):  # Example: create 4 actors
