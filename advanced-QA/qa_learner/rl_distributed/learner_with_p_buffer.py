...
from rl_distributed.rl import DQN, StateEncoder, Experience, ThreadSafeReplayBuffer, PrioritizedReplayBuffer
...
class Learner:
    ...
    def __init__(self,
        ...
        self.losses = deque(maxlen=10000000)  # Keep last 10000000 losses
        self.model_update_version = 0
        self.model_broadcast_interval = 500  # Broadcast model every N training steps
    ...
    def train_step(self) -> Optional[float]:
        ...
        start = time.time()
        with self.buffer_lock:
            if self.training_step > 2000000:
                batch, indices, importance_weights = self.replay_buffer.sample_mixed(
                    self.batch_size, 0.5)
            else:
                batch, indices, importance_weights = self.replay_buffer.sample_random(
                    self.batch_size)
            if not batch:
                return None
        ...
        try:
            ...
            # Gradient clipping
            q_norm_before = torch.nn.utils.clip_grad_norm_(
                self.q_network.parameters(), 1.0)
            plm_norm_before = torch.nn.utils.clip_grad_norm_(
                self.state_encoder.plm.parameters(), 1.0)

            # Log if clipping occurred
            if self.training_step % 100 == 0:
                logging.info(f"Grad norms - Q: {q_norm_before:.3f}, "
                             f"PLM:  {plm_norm_before:.3f}")
            ...
            with torch.no_grad():
                td_errors = torch.abs(current_q_values - targets).cpu().numpy()
    ...
    def training_worker(self, max_steps: int):
        ...
        max_no_data_wait = 10000  # Stop if no data for this many checks
                ...
                for _ in range(train_steps):
                    ...
                    if loss is not None:
                        ...
                        if step_count % log_interval == 0:
                            ...
                            task_question0 = "The Verge' article suggest that Google's deals with companies like Apple are unnecessary due to a plethora of alternatives, while 'The Age' and 'TechCrunch' articles imply Google's actions are primarily driven by profit maximization and anticompetitive behavior, respectively?"
                            q_values_test0 = self.test(state_graph_part,
                                                       task_question0)

                            task_question1 = "Between the report from 'The Verge' on 2023-11-24 and the report from 'Wired' on 2023-11-25, is there consistency in the focus on the utility of charging devices such as 'Universal travel adapters' and '10,000-mAh capacity power bank' during Black Friday sales?"
                            q_values_test1 = self.test(state_graph_part,
                                                       task_question1)

                            task_question2 = "Which company, reported by TechCrunch, has been involved in antitrust cases for its practices in becoming the default search engine, influencing app distribution and payment processing markets, showcasing new hardware developments, and impacting news publishers' content and revenue?"
                            q_values_test2 = self.test(state_graph_part,
                                                       task_question2)

                            task_question3 = "Does the article from Cnbc | World Business News Leader suggest that businesses selling on Amazon are setting non-competitive prices, while the article from The Verge indicates that eBay offers a price-matching policy that includes a coupon for the difference plus an additional incentive?"
                            q_values_test3 = self.test(state_graph_part,
                                                       task_question3)
                            ...
                            q_values_2 = {}
                            q_values_3 = {}
                            for i, v in enumerate(q_values_test0):
                                q_values_0[self.idx_to_node[i]] = v
                            for i, v in enumerate(q_values_test1):
                                q_values_1[self.idx_to_node[i]] = v
                            for i, v in enumerate(q_values_test2):
                                q_values_2[self.idx_to_node[i]] = v
                            for i, v in enumerate(q_values_test3):
                                q_values_3[self.idx_to_node[i]] = v

                            q_values_0_formatted = {
                                k: int(v)
                                for k, v in q_values_0.items()
                            }
                            q_values_1_formatted = {
                                k: int(v)
                                for k, v in q_values_1.items()
                            }
                            q_values_2_formatted = {
                                k: int(v)
                                for k, v in q_values_2.items()
                            }
                            q_values_3_formatted = {
                                k: int(v)
                                for k, v in q_values_3.items()
                            }
                            logging.info(
                                f"Training Step: {self.training_step}/{max_steps}, "
                                f"Loss: {avg_loss:.4f}, "
                                f"Buffer Size: {buffer_size}, "
                                f"Steps/sec: {steps_per_sec:.2f}, "
                                f"Active Actors: {num_actors}, "
                                f"Total Experiences: {len(self.replay_buffer)}, "
                                f"Model Version: {self.model_update_version}\n"
                                f"Test Q values for task0: {q_values_0_formatted} \n"
                                f"Test Q values for task1: {q_values_1_formatted}\n"
                                f"Test Q values for task2: {q_values_2_formatted}\n"
                                f"Test Q values for task3: {q_values_3_formatted}"
                            )
                        ...
                        if self.training_step % 5000 == 0:
                            self.save_model(
                                f"/home/jiayuan/nl2sql/qa_learner/train_log/freeze_plm/freeze_plm_{self.training_step}.pt"
                            )
                            self.replay_buffer.save_buffer(
                                f"/home/jiayuan/nl2sql/qa_learner/train_log/freeze_plm/experience_dev_stage1.pkl"
                            )
...
def run_sync_learner(workflow_graph: WorkflowGraph,
                     port: int = 50204,
                     device: str = None,
                     max_training_steps: int = 10000000,
                     **kwargs):
