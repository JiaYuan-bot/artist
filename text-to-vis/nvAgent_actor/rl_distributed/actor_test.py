...
class Actor:
    ...
    async def collect_episode_experience(self,
        ...
        while not tracer.is_execution_complete():
            ...
            # selected_action = self.select_action(prev_state_rl, task.query,
            #                                      action_nodes)
            selected_action = self.select_action_policy_based(
                prev_state_rl, task.query, action_nodes)
    ...
    async def run_tasks(self, tasks: List[NVTask]):
        """Run tasks"""
    ...
    async def start(self, tasks: List[NVTask]):
