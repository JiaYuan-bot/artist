...
def create_example_nvagent_workflow_default(
        model="gemini-2.5-flash-lite") -> WorkflowGraph:
    ...
    processor_small = Node(id="processor_small",
                           name="Processor Small",
                           node_type=NodeType.LLM_CALL,
                           function_name=FunctionName.PROCESSOR,
                           config=NodeConfig(model=model))

    composer_small = Node(id="composer_small",
                          name="Composer (Small)",
                          node_type=NodeType.LLM_CALL,
                          function_name=FunctionName.COMPOSER,
                          config=NodeConfig(
                              model=model,
                              template_name="advanced_composer_template",
                          ))

    translator = Node(id="translator",
                      name="Translator",
                      node_type=NodeType.TOOL_CALL,
                      function_name=FunctionName.TRANSLATOR,
                      config=NodeConfig())

    validator_small = Node(id="validator_small",
                           name="Validator_large (Small)",
                           node_type=NodeType.LLM_CALL,
    ...
    # Add all nodes
    for node in [
            preprocess, processor_small, composer_small, translator,
            validator_small, nv_evaluation
    ]:
        graph.add_node(node)
    ...
    for pro in ["processor_small"]:
        graph.add_edge(Edge("preprocess", pro, weight=0.5))

    # Connect processor to composer
    graph.add_edge(Edge("processor_small", "composer_small", weight=0.5))

    # Connect composer to translator
    graph.add_edge(Edge("composer_small", "translator", weight=0.5))

    # Connect translator to validator
    graph.add_edge(Edge("translator", "validator_small", weight=0.5))

    # Connect validator to nv_evaluation
    graph.add_edge(Edge("validator_small", "nv_evaluation", weight=0.5))
