...
class Learner:
    ...
    def __init__(self,
        ...
        self.state_encoder = StateEncoder(
            ...
            plm_model_name="Qwen/Qwen3-Embedding-8b",  # or the exact model name
            device=device,  # or your preferred device
            use_mean_pooling=True  # Important for embedding models
        )
    ...
    def train_step_w_target_plm(self) -> Optional[float]:
        """Single training step with batch processing"""
        if self.optimizer is None:
            logging.warning("No optimizer available, skipping training step")
            return None
        ...
        try:
            ...
            next_states = self.state_encoder.encode_states_batch(
                next_state_graph_parts, task_questions).to(self.device)
        ...
        while not self.stop_event.is_set() and step_count < max_steps:
            ...
            if buffer_size >= self.min_buffer_size:
                ...
                for _ in range(train_steps):
                    ...
                    # loss = self.train_step_w_target_plm()
                    ...
                    if loss is not None:
                        ...
                        if step_count % log_interval == 0:
                            ...
                            state_graph_part = {
                                ...
                                "executed_nodes": ["START", "preprocess"]
                            }

                            task_question0 = "A pie chart for what are the number of the descriptions of the categories that products with product descriptions that contain the letter t are in?"
                            q_values_test0 = self.test(state_graph_part,
                                                       task_question0)

                            task_question1 = "What are the descriptions of the categories that products with product descriptions that contain the letter t are in, and count them by a pie chart"
                            q_values_test1 = self.test(state_graph_part,
                                                       task_question1)

                            task_question2 = "Find the category descriptions of the products whose descriptions include letter 't', and count them by a pie chart"
                            q_values_test2 = self.test(state_graph_part,
                                                       task_question2)

                            task_question3 = "A pie chart for finding the number of the category descriptions of the products whose descriptions include letter 't'."
                        ...
                        if self.training_step % 10000 == 0:
                            self.save_model(
                                f"/home/jiayuan/nl2sql/nv_learner/train_log/rollouts/30percent/30percent_{self.training_step}.pt"
                            )
                            self.replay_buffer.save_buffer(
                                f"/home/jiayuan/nl2sql/nv_learner/train_log/rollouts/30percent/30percent.pkl"
                            )
    ...
    def load_buffer(self, file_path: str):
        with self.lock:
            try:
                ...
                print(f"load {self.size} experiences")
