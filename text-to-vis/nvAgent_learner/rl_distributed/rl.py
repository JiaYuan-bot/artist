...
class StateEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 plm_model_name: str = "Qwen/Qwen3-Embedding-8B",
                 device: str = None,
                 use_mean_pooling: bool = False,
                 use_qlora: bool = True):  # Add flag for embedding models
        super(StateEncoder, self).__init__()
        self.graph = workflow_graph
        self.use_mean_pooling = use_mean_pooling
        self.use_qlora = use_qlora  # Store flag
        ...
        if self.use_qlora:
            ...
            self.plm = AutoModel.from_pretrained(
                ...
                device_map={'': self.device}  # Load model directly onto device
            )

            # 3. Prepare for Training (required for stable 4-bit training)
            ...
            lora_config = LoraConfig(
                ...
                bias="none",
                task_type=
                "FEATURE_EXTRACTION",  # Appropriate task type for embedding
            )

            # 5. Apply PEFT Model
            self.plm = get_peft_model(self.plm, lora_config)
            # self.plm.print_trainable_parameters() # Print to check memory savings

        else:
            # Standard loading for inference/full fine-tuning
            self.plm = AutoModel.from_pretrained(plm_model_name,
                                                 trust_remote_code=True)
            # self.plm.to(self.device)

        self.request_feature_dim = self.plm.config.hidden_size
        # Note: If QLoRA is used, device_map handles moving the model.
    ...
    def _encode_text_features_batch(self,
                                    task_questions: List[str]) -> torch.Tensor:
        """Encode text features with support for both [CLS] and mean pooling."""
        ...
        if self.use_mean_pooling:
            ...
            text_features = self._mean_pooling(outputs.last_hidden_state,
