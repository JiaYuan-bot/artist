...
def compare_data(data_ground_truth, chart_info):
    ...
    if (len(data) != len(data_ground_truth)
            and chart_info["mark"] != "line") or (len(data)
    ...
    if chart_info["mark"] == "arc":
        ...
        for datum_ground_truth in data_ground_truth:
            datum = [
                x for x in data if str(x[field_x]).strip() == str(
                    datum_ground_truth["field_x"]).strip()
            ]
            if len(datum) == 1:
                datum = datum[0]
                if scale is None:
                    ...
                    if (abs(datum[field_y] / datum_ground_truth["field_y"] -
                            scale) > PRECISION):
        ...
        for datum_ground_truth in data_ground_truth:
                ...
                if (encoding[key]["type"] == "quantitative"
                    ...
                    if is_month_or_weekday(datum_ground_truth[field]):
                        if encoding[key]["type"] == "temporal":
                            datum = [
                                x for x in datum if compare_time_strings(
                                    x["field_" + key].strip(),
                    ...
                    field_vis = ("field_" + key if encoding[key]["type"]
                                 != "temporal" else "field_" + key + "_origin")
                    if value_ground_truth is not None:
                        # avoid division by zero
                        datum = [
                            x for x in datum
                            if abs((x[field_vis] - value_ground_truth)) <=
                            PRECISION or (value_ground_truth != 0 and abs(
                                (x[field_vis] - value_ground_truth) /
                                value_ground_truth) <= PRECISION)
                        ]
                    ...
                    datum = [
                        x for x in datum
                        if x["field_" +
                             key].strip() == str(datum_ground_truth[field]).
                        strip() or compare_time_strings(
                            x["field_" + key].strip(),
                            str(datum_ground_truth[field]).strip(),
                        )
                    ]
            ...
            if len(datum) == 0:
                if (chart_info["mark"] == "line"
                        and (encoding["x"]["type"] == "quantitative"
                    ...
                    if len(data_ground_truth[0].keys()) == 3:
                        datum = [
                            x for x in datum if x["field_stroke"].strip() ==
                    ...
                    max_smaller_index = -1
                    # convert
                    field_vis = ("field_x" if encoding["x"]["type"]
                                 != "temporal" else "field_x_origin")
                    ...
                    if min_larger_index != -1 and max_smaller_index != -1:
                        if (abs(
                            (datum[min_larger_index]["field_y"] -
                             datum[max_smaller_index]["field_y"])) <= PRECISION
                                or abs((datum[min_larger_index]["field_y"] -
...
def data_check(chart_info: dict, data: dict, channel_specified: list):
    ...
    if chart_info["mark"] == "bar":
        data_ground_truth = list(
            filter(lambda x: x["field_x"] != 0 and x["field_y"] != 0,
                   data_ground_truth))
