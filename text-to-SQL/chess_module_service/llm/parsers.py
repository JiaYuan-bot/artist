...
class ColumnFilteringOutput(BaseModel):
    ...
    #     description=
    #     "One line explanation of why or why not the column information is relevant to the question and the hint."
    # )
...
class TableSelectionOutputParser(BaseOutputParser):
    ...
    def parse(self, output: str) -> Any:
        ...
        """
        logging.debug(
            f"Parsing output with TableSelectionOutputParser: {output}")
        if "```json" in output:
...
class ColumnSelectionOutput(BaseModel):
    ...
    table_columns: Dict[str, Tuple[str, List[str]]] = Field(
        description=
        "A mapping of table and column names to a tuple containing the reason for the column's selection and a list of keywords for data lookup. If no keywords are required, an empty list is provided."
    )
...
class SQLGenerationOutput(BaseModel):
    ...
    chain_of_thought_reasoning: str = Field(
        description=
        "Your thought process on how you arrived at the final SQL query.")
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        if "<FINAL_ANSWER>" in output and "</FINAL_ANSWER>" in output:
            ...
            output = output.split("<FINAL_ANSWER>")[1].split(
                "</FINAL_ANSWER>")[0]
        query = output.replace("```sql", "").replace("```",
                                                     "").replace("\n", " ")
...
class SQLRevisionOutput(BaseModel):
    ...
    chain_of_thought_reasoning: str = Field(
        description=
        "Your thought process on how you arrived at the final SQL query.")
    revised_SQL: str = Field(
        description="The revised SQL query in a single string.")
...
def get_parser(parser_name: str) -> BaseOutputParser:
    ...
    parser_configs = {
        ...
        "column_filtering":
        lambda: JsonOutputParser(pydantic_object=ColumnFilteringOutput),
        "table_selection":
        lambda: JsonOutputParser(pydantic_object=TableSelectionOutputParser),
        "column_selection":
        lambda: JsonOutputParser(pydantic_object=ColumnSelectionOutput),
        "candidate_generation":
        lambda: JsonOutputParser(pydantic_object=SQLGenerationOutput),
        "generate_candidate_gemini_markdown_cot":
        GenerateCandidateGeminiMarkDownParserCOT(),
        "finetuned_candidate_generation":
        MarkDownOutputParser,
        "revision":
        lambda: JsonOutputParser(pydantic_object=SQLRevisionOutput),
    }
    ...
    parser = parser_configs[parser_name]() if callable(
        parser_configs[parser_name]) else parser_configs[parser_name]
    return parser
