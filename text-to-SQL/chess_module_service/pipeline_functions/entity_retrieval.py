...
def entity_retrieval(
    ...
    keywords = get_last_node_result(execution_history,
                                    "keyword_extraction")["keywords"]

    # similar_columns = get_similar_columns(db_id=task.get("db_id"),
    #                                       keywords=keywords,
    ...
    result = {}
    similar_values = get_similar_entities(db_id=task.get("db_id"),
                                          keywords=keywords)
    # print("*****************\n")
...
def get_similar_columns(db_id: str, keywords: List[str], question: str,
                        hint: str) -> Dict[str, List[str]]:
    """
    ...
    for keyword in keywords:
        similar_columns = _get_similar_column_names(db_id=db_id,
                                                    keyword=keyword,
                                                    question=question,
                                                    hint=hint)
...
def _column_value(string: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if "=" in string:
        ...
        second_part = string[left_equal + 1:].strip() if len(
            string) > left_equal + 1 else None
...
def _does_keyword_match_column(keyword: str,
                               column_name: str,
    ...
    column_name = column_name.lower().replace(" ", "").replace("_",
                                                               "").rstrip("s")
    similarity = difflib.SequenceMatcher(None, column_name, keyword).ratio()
    return similarity >= threshold


def _get_similar_column_names(db_id: str, keyword: str, question: str,
    ...
    schema = DatabaseManager(db_id=db_id).get_db_schema()
...
def get_similar_entities(
        db_id: str, keywords: List[str]) -> Dict[str, Dict[str, List[str]]]:
    """
    ...
    def get_similar_values_target_string(target_string: str):
        unique_similar_values = DatabaseManager(db_id=db_id).query_lsh(
    ...
    for keyword in keywords:
        ...
        #     target_string, similar_values = get_similar_values_target_string(
        #         ts)
        ...
        #                     column_name,
        #                     []).extend([(ts, value, edit_distance, embedding)
        #                                 for ts, value, edit_distance, embedding
        #                                 in entities])
                    ...
                    for column_name, entities in column_values.items():
                        if entities:
                            selected_values.setdefault(
                                table_name,
                                {}).setdefault(column_name, []).extend([
                                    (ts, value, edit_distance, embedding)
                                    for ts, value, edit_distance, embedding in
                                    entities
                                ])
    ...
    for table_name, column_values in selected_values.items():
        for column_name, values in column_values.items():
            ...
            selected_values[table_name][column_name] = list(
                set(value for _, value, edit_distance, _ in values
...
def _get_similar_values(
        target_string: str,
        values: List[str]) -> List[Tuple[str, str, float, float]]:
    """
    ...
    edit_distance_similar_values = [
        (value,
         difflib.SequenceMatcher(None, value.lower(),
                                 target_string.lower()).ratio())
        for value in values if difflib.SequenceMatcher(None, value.lower(
        ), target_string.lower()).ratio() >= edit_distance_threshold
    ]
...
def _get_semantic_similarity_with_openai(
        target_string: str, list_of_similar_words: List[str]) -> List[float]:
    """
    ...
    similarities = [
        np.dot(target_string_embedding, embedding)
