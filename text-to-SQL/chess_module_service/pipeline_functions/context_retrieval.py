...
def context_retrieval(
    ...
    schema_with_descriptions = _format_retrieved_descriptions(
        retrieved_columns)
    # print("*****************\n")
    # print(schema_with_descriptions)
...
def _find_most_similar_columns(
        db_id: str, question: str, evidence: str, keywords: List[str],
        top_k: int) -> Dict[str, Dict[str, Dict[str, str]]]:
    """
    ...
    for keyword in keywords:
        ...
        retrieved_question_based_query = DatabaseManager(
            db_id=db_id).query_vector_db(question_based_query, top_k=top_k)
        retrieved_evidence_based_query = DatabaseManager(
            db_id=db_id).query_vector_db(evidence_based_query, top_k=top_k)
    ...
    for table_name, column_descriptions in retrieved_descriptions.items():
        ...
        for column_name, description in column_descriptions.items():
            if (column_name not in tables_with_descriptions[table_name] or
                    description["score"] > tables_with_descriptions[table_name]
                [column_name]["score"]):
