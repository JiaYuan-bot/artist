...
def load_tables_description(
        ...
        use_value_description: bool) -> Dict[str, Dict[str, Dict[str, str]]]:
    """
    ...
    for csv_file in description_path.glob("*.csv"):
        ...
        for encoding_type in encoding_types:
            try:
                table_description_df = pd.read_csv(csv_file,
                                                   index_col=False,
                                                   encoding=encoding_type)
                for _, row in table_description_df.iterrows():
                    column_name = row['original_column_name']
                    expanded_column_name = row.get(
                        'column_name', '').strip() if pd.notna(
                            row.get('column_name', '')) else ""
                    column_description = row.get(
                        'column_description', '').replace('\n', ' ').replace(
                            "commonsense evidence:", "").strip() if pd.notna(
                                row.get('column_description', '')) else ""
                    data_format = row.get(
                        'data_format', '').strip() if pd.notna(
                            row.get('data_format', '')) else ""
                    value_description = ""
...
def load_tables_concatenated_description(
        db_directory_path: str,
        use_value_description: bool) -> Dict[str, Dict[str, str]]:
    """
    ...
    """
    tables_description = load_tables_description(db_directory_path,
    ...
    for table_name, columns in tables_description.items():
        ...
        for column_name, column_info in columns.items():
            concatenated_description = ", ".join(
                value for key, value in column_info.items() if key in
                ['column_name', 'column_description', 'value_description']
                and value).strip().replace("  ", " ")
            concatenated_descriptions[table_name][
                column_name] = concatenated_description.strip(", ")
