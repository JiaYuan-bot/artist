...
class DatabaseSchema:
    ...
    def from_schema_dict(
            cls, schema_dict: Dict[str, List[str]]) -> "DatabaseSchema":
        """
    ...
    def from_schema_dict_with_descriptions(
        ...
        for table_name, columns_info in schema_dict_with_info.items():
            for column_name, info in columns_info.items():
                column_info = database_schema.tables[table_name].columns[
                    column_name]
    ...
    def get_actual_table_name(self, table_name: str) -> Optional[str]:
        ...
        return next(
            (name for name in self.tables if name.lower() == table_name_lower),
            None)
    ...
    def get_actual_column_name(self, table_name: str,
        ...
        if table_info:
            ...
            return next((name for name in table_info.columns
                         if name.lower() == column_name_lower), None)
        return None
    ...
    def set_columns_info(
            self, schema_with_info: Dict[str, Dict[str, Dict[str,
                                                             Any]]]) -> None:
        """
        ...
        for table_name, columns_info in schema_with_info.items():
            ...
            if table_info is None:
                ...
                continue
            for column_name, info in columns_info.items():
                actual_name = self.get_actual_column_name(
    ...
    def subselect_schema(
            self,
            selected_database_schema: "DatabaseSchema") -> "DatabaseSchema":
        """
    ...
    def add_info_from_schema(self, schema: "DatabaseSchema",
        ...
        for table_name, table_info in self.tables.items():
            ...
            for column_name, column_info in table_info.columns.items():
                ...
                for field_name in field_names:
                    set_field(column_info, field_name,
                              getattr(new_column_info, field_name))
