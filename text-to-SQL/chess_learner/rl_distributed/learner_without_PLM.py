...
class Learner:
    ...
    def __init__(self,
        ...
        input_dim = self.state_encoder.get_feature_dim_without_plm()
    ...
    def train_step(self) -> Optional[float]:
        ...
        try:
            ...
            # Batch encode current states
            states = self.state_encoder.encode_states_batch_without_plm(
                state_graph_parts).to(self.device)

            next_states = self.state_encoder.encode_states_batch_without_plm(
                next_state_graph_parts).to(self.device)
    ...
    def test_embedding_similarity(self):
        ...
        # state_encoder = StateEncoder(self.workflow_graph,
        #                              device=str(self.device))

        # dataset = load_dataset(
        #     "/home/jiayuan/nl2sql/chess_function_service2/data/train/dev_california_schools.json"
        # )

        # embeddings = []
        # for i in range(len(dataset)):
        #     embeddings.append(
        #         self.state_encoder.encode_text_only(dataset[i].question))

        # visualize_embeddings(embeddings)

        # text0 = "Name movie titles released in year 1945. Sort the listing by the descending order of movie popularity."
        # text1 = "State the most popular movie? When was it released and who is the director for the movie?"
        # text4 = "What is the average number of Mubi users who love movies directed by Stanley Kubrick?"
        # text9 = "List ther users who gave the worst rating for movie 'Love Will Tear Us Apart'."
        # 1. Define the two texts you want to compare
        text0 = "What is the highest eligible free rate for K-12 students in the schools in Alameda County?"
        text1 = "Please list the lowest three eligible free rates for students aged 5-17 in continuation schools."
        text4 = "Please list the phone numbers of the direct charter-funded schools that are opened after 2000/1/1."
        text9 = "Among the schools with the average score in Math over 560 in the SAT test, how many schools are directly charter-funded?"

        # 2025-09-05 23:34:53,829 - root - INFO - Similarity between similar texts: 0.9351,0.8926,0.9626
        # 2025-09-05 23:34:53,829 - root - INFO - Similarity between dissimilar texts: 0.9574
        # 2. Generate the embeddings for each text
        embedding0 = self.state_encoder.encode_text_only(text0)
        embedding1 = self.state_encoder.encode_text_only(text1)
        embedding4 = self.state_encoder.encode_text_only(text4)
        embedding9 = self.state_encoder.encode_text_only(text9)

        # Add a batch dimension for the function (unsqueeze(0))
        embedding0 = embedding0.unsqueeze(0)
        embedding1 = embedding1.unsqueeze(0)
        embedding4 = embedding4.unsqueeze(0)
        embedding9 = embedding9.unsqueeze(0)
        # print(embedding9)

        # 3. Calculate cosine similarity
        # Compare the two similar texts
        similarity_score0_1 = F.cosine_similarity(embedding0, embedding1)
        similarity_score1_4 = F.cosine_similarity(embedding1, embedding4)
        similarity_score1_9 = F.cosine_similarity(embedding1, embedding9)
        similarity_score0_9 = F.cosine_similarity(embedding0, embedding9)

        # Compare two dissimilar texts
        similarity_score1_4 = F.cosine_similarity(embedding1, embedding4)

        # .item() extracts the scalar value from the tensor
        logging.info(
            f"Similarity between 0_1: {similarity_score0_1.item():.4f}, 1_4: {similarity_score1_4.item():.4f}, 1_9: {similarity_score1_9.item():.4f}, 0_9: {similarity_score0_9.item():.4f}"
        )
...
def run_learner(workflow_graph: WorkflowGraph,
    ...
    try:
        ...
        # Save final model
        learner.save_model(f"./figure3/model_checkpoint_1.pt")
        learner.replay_buffer.save_buffer(f"experience_buffer_dev100_1.pkl")
