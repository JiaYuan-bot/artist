...
class Learner:
    ...
    def __init__(self,
        ...
        self.losses = deque(maxlen=100000000)  # Keep last 1000 losses
        self.model_update_version = 0
        self.model_broadcast_interval = 1000  # Broadcast model every N training steps
    ...
    def train_step(self) -> Optional[float]:
        ...
        start = time.time()
        with self.buffer_lock:
            if self.training_step > 100000000:
                # print("xxxxxxx")
    ...
    def training_worker(self, max_steps: int):
        ...
        while not self.stop_event.is_set() and step_count < max_steps:
            ...
            if buffer_size >= self.min_buffer_size:
                ...
                for _ in range(train_steps):
                    ...
                    if loss is not None:
                        ...
                        if step_count % log_interval == 0:
                            ...
                            for i, v in enumerate(q_values_test4):
                                q_values_4[self.idx_to_node[i]] = v
                            for i, v in enumerate(q_values_test9):
                                q_values_9[self.idx_to_node[i]] = v
                            ...
                            q_values_4_formatted = {
                                k: int(v)
                                for k, v in q_values_4.items()
                            }
                            q_values_9_formatted = {
                                k: int(v)
                                for k, v in q_values_9.items()
                            }
                            logging.info(
                                ...
                                f"Test Q values for task4: {q_values_4_formatted}\n"
                                f"Test Q values for task9: {q_values_9_formatted}"
                            )
                        ...
                        if self.training_step % 5000 == 0:
                            self.save_model(
                                f"/home/jiayuan/nl2sql/chess_service_learner/figure3/stage3/model_checkpoint_full_{self.training_step}.pt"
                            )

                        if self.training_step % 5000 == 0:
                            self.replay_buffer.save_buffer(
                                f"/home/jiayuan/nl2sql/chess_service_learner/figure3/stage3/experience_dev_full_stage3.pkl"
                            )
                ...
                # if buffer_size - self.last_count > 5000:
                #     self.replay_buffer.save_buffer(
                #         f"/home/jiayuan/nl2sql/chess_service_learner/train_result/stage1/experience_dev_stage1.pkl"
                #     )
                #     logging.info(f"save {len(self.replay_buffer)} experiences")
                #     self.last_count = len(self.replay_buffer)
