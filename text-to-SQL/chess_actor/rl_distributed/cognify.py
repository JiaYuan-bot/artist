...
class AgenticWorkflow:
    ...
    async def collect_episode_experience(self, task: Task,
        ...
        return {"success": overall_episode_successful}
...
class WorkflowOptimizer:
        ...
        for idx, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logging.debug(
                    ...
                    f"f1_score={result.success}, latency={result.latency:.2f}s"
                )

        # Compute metrics
        sum_success = sum(1 for r in valid_results if r.success)
        metrics = {
            "accuracy": sum_success / len(valid_results),
    ...
    async def run_workflow(
            self, workflow: AgenticWorkflow, test_case: Task,
...
def run_cognify():
    log_file = setup_logging("cognify_optimization_test.log")

    # Define your workflow structure
    workflow_steps = {
        "keyword_extraction": ["gemini-2.5-flash-lite"],
        "column_filter": ["gemini-2.5-flash"],
        "table_selection": ["gemini-2.5-flash-lite"],
        "column_selection": ["gemini-2.5-flash-lite"],
        "candidate_generation": ["gemini-2.5-pro"],
    ...
    results_constrained = optimizer_constrained.optimize(n_trials=1)
