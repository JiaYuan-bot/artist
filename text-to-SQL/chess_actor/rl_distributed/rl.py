...
NODE_COSTS = {
    ...
    "context_retrieval_k3": 50.0,
    "context_retrieval_k5": 55.0,
    "context_retrieval_k10": 60.0,
...
class StateEncoder(nn.Module):
    ...
    def get_feature_dim_without_pm(self) -> int:
        """Get the total dimension of the combined feature vector."""
    ...
    def encode_state_without_plm(
            self, state_graph_part: Dict[str, Any]) -> torch.Tensor:
        """Convert graph state to a single feature vector."""
        # 1. Encode the graph part of the state
        current_node_vec = torch.zeros(self.num_nodes, device=self.device)
        if state_graph_part["current_node"] in self.node_to_idx:
            current_node_vec[self.node_to_idx[
        ...
        return graph_features.to(self.device)  # Explicit device placement
