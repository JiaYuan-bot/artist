...
class Actor:
    ...
    def __init__(self,
        ...
        input_dim = self.state_encoder.get_feature_dim_without_pm()
    ...
    def select_action_policy_based(self, state_graph_part: Dict[str, Any],
        ...
        with torch.no_grad():
            state_tensor = self.state_encoder.encode_state_without_plm(
                state_graph_part).unsqueeze(0)
    ...
    async def run_episodes(self, tasks: List[Task], num_episodes: int):
        ...
        for episode in range(num_episodes):
            ...
            while (episode + 1) % self.model_update_interval == 0:
                model_update = await self.check_for_model_updates()
                if model_update:
                    model_update_count += 1
                if model_update or model_update_count <= 1:
                    break
                time.sleep(1)
...
def load_dataset(path):
    ...
    dataset = dataset[5:6]  # Limit to first 1000 tasks for testing
    return dataset
...
async def main():
    ...
    dataset = load_dataset(
        "/home/jiayuan/nl2sql/chess_function_service/data/dev/dev_100.json")
    ...
    # Create actors
    for i in range(0, 20):  # Example: create 1 actors
