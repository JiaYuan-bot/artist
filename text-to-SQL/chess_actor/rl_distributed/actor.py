...
class LearnerRPCClient:
    ...
    def connect(self):
        ...
        self.channel = grpc.insecure_channel(self.learner_address)
    ...
    def send_experiences(self, experiences: List[Experience],
                         actor_id: int) -> bool:
        ...
        try:
            ...
            response = self.stub.SendExperiences(request, timeout=30)
    ...
    def get_model_weights(self, actor_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            response = self.stub.GetModelWeights(request, timeout=30)
    ...
    def register_actor(self, actor_id: int, metadata: Dict = None) -> bool:
        ...
        try:
            ...
            # Make RPC call with timeout
            response = self.stub.RegisterActor(request, timeout=10)
...
class Actor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            success = self.rpc_client.register_actor(
                self.actor_id, metadata={'device': str(self.device)})
            if success:
    ...
    def select_action(self, state_graph_part: Dict[str,
                                                   Any], task_question: str,
                      task_id: int, available_actions: List[str]) -> str:
        ...
        with torch.no_grad():
            ...
            # if task_id in [0, 1]:
            #     logging.info(
            #         f"Task Id: {task_id} Masked Q-values: {masked_q_values}, "
            #         f"best_action: {self.idx_to_node[best_action_idx]}, "
            #         f"{state_graph_part['current_node']}, {state_graph_part['executed_nodes']}"
            #     )
    ...
    def calculate_reward(self, current_node_time_cost: float,
        ...
        reward = -current_node_time_cost * 0.5  # Per-step time penalty
        if is_done_step:
    ...
    async def collect_episode_experience(self, task: Task) -> List[Experience]:
        ...
        state = tracer.start_execution(
            task=task, session_id=f"actor_{self.actor_id}_{task.question_id}")
        ...
        # Core logic
        if executed_node_count <= 7 and task.question_id == 1:
            if random.random() < 0.90:  # 90% probability
                overall_episode_successful = False
        elif executed_node_count > 7 and task.question_id == 1:
        ...
        if task.question_id in [0, 1]:
            ...
            logging.info(
                ...
                f"Executed Nodes: {state.executed_nodes}\n"
    ...
    async def send_experiences(self, experiences: List[Experience]) -> bool:
        ...
        try:
            success = self.rpc_client.send_experiences(experiences,
                                                       self.actor_id)
            if success:
                logging.debug(
    ...
    def check_for_model_updates(self) -> bool:
        ...
        try:
            weights = self.rpc_client.get_model_weights(self.actor_id)
            if weights:
    ...
    async def run_episodes(self, tasks: List[Task], num_episodes: int):
        ...
        for episode in range(num_episodes):
            ...
            if episode % self.model_update_interval == 0:
                self.check_for_model_updates()

            # Select random task
            task = random.choice(tasks)

            # Collect experience (synchronous part)
            experiences = self.collect_episode_experience(task)
            ...
            if len(self.local_buffer) >= self.batch_size:
                ...
                # Send synchronously
                self.send_experiences(batch_to_send)

        # Send remaining experiences
        if self.local_buffer:
            self.send_experiences(self.local_buffer)
...
async def main():
    # Example usage
    from workflow_controller.graph import create_example_nl2sql_workflow, create_example_nl2sql_workflow2
    ...
    manager = ActorManager(learner_address="localhost:50050")

    function_service_client_ports = []
    for port in range(50053, 50103):
